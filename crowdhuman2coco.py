...
def crowdhuman2coco(args, odgt_path, json_path , data_path):
    ...
    print("start convert")
    counter = 0
    print(f"Total {len(records)} images in CrowdHuman dataset")
    for image_id, image_dict in enumerate(records):
        ...
        for _, instance in enumerate(gt_box):
            ...
            if args.full:
                attr = instance['extra']
                if args.rm_occ and attr.get('ignore', 0):  # Default to 0 if 'ignore' is missing
                    continue
        ...
        counter += 1
        print(f"{counter}/{len(records)} images have been converted")
